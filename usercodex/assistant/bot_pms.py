...
@codex.bot_cmd(incoming=True, func=lambda e: e.is_private)
    ...
    if chat.id != Config.OWNER_ID:
        ...
        if user_id is not None:
            try:
                ...
                return await event.reply("𝗧𝗵𝗶𝘀 𝗯𝗼𝘁 𝘄𝗮𝘀 𝗯𝗹𝗼𝗰𝗸𝗲𝗱 𝗯𝘆 𝘁𝗵𝗲 𝘂𝘀𝗲𝗿.o")
            except Exception as e:
...
async def send_flood_alert(user_) -> None:
    ...
    if FloodConfig.ALERT and (user_.id in FloodConfig.ALERT.keys()):
        ...
        try:
            ...
            if BOTLOG:
                await codex.tgbot.send_message(
    ...
    if found:
        if flood_count >= FloodConfig.AUTOBAN:
            if user_.id in Config.SUDO_USERS:
                ...
                if BOTLOG:
                    await codex.tgbot.send_message(BOTLOG_CHATID, sudo_spam)
            else:
        ...
        if not fa_id:
            return
        try:
            msg_ = await codex.tgbot.get_messages(BOTLOG_CHATID, fa_id)
        ...
        if BOTLOG:
            fa_msg = await codex.tgbot.send_message(
                BOTLOG_CHATID,
                flood_msg,
                buttons=buttons,
            )
        try:
            chat = await codex.tgbot.get_entity(BOTLOG_CHATID)
            await codex.tgbot.send_message(
                Config.OWNER_ID,
            ...
            if BOTLOG:
                await codex.tgbot.send_message(BOTLOG_CHATID, "**Unblock your bot !**")
...
async def bot_pm_ban_cb(c_q: CallbackQuery):
    ...
    try:
        user = await codex.get_entity(user_id)
    except Exception as e:
...
@codex.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@codex.bot_cmd(incoming=True, func=lambda e: e.is_private)
@codex.bot_cmd(edited=True, func=lambda e: e.is_private)
