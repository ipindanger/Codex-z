...
def higlighted_text(
    ...
    if font_name is None:
        font_name = "usercodex/helpers/styles/impact.ttf"
...
async def hide_inlinebot(borg, bot_name, text, chat_id, reply_to_id, c_lick=0):
    ...
    cod = await sticcers[c_lick].click("me", hide_via=True)
    if cod:
        await borg.send_file(int(chat_id), cod, reply_to=reply_to_id)
        await cod.delete()
...
async def waifutxt(text, chat_id, reply_to_id, bot):
    ...
    cod = await sticcers[0].click("me", hide_via=True)
    if cod:
        await bot.send_file(int(chat_id), cod, reply_to=reply_to_id)
        await cod.delete()
