...
NO_ADMIN = "`I am not an admin stupid! `"
...
async def promote(event):
    ...
    if not user:
        return
    codevent = await edit_or_reply(event, "`Promoting...`")
    try:
...
async def _ban_person(event):
    ...
    codevent = await edit_or_reply(event, "`Whacking the pest!`")
    try:
...
async def nothanos(event):
    ...
    if not user:
        return
    codevent = await edit_or_reply(event, "`Unbanning...`")
    try:
