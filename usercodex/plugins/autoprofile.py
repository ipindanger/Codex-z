...
autopic_path = os.path.join(os.getcwd(), "usercodex", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "usercodex", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "usercodex", "photo_pfp.png")
...
async def custompfploop():
    ...
    while CUSTOMPICSTART:
        ...
        file = await codex.upload_file("donottouch.jpg")
        try:
            if i > 0:
                await codex(
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        img = Image.open(autophoto_path)
        drawn_text = ImageDraw.Draw(img)
        cod = str(base64.b64decode("dXNlcmJvdC9oZWxwZXJzL3N0eWxlcy9kaWdpdGFsLnR0Zg=="))[
            2:36
        ]
        fnt = ImageFont.truetype(cod, 200)
...
async def autopfp_start():
    ...
    while AUTOPFP_START:
        ...
        i += 1
        await codex(functions.photos.UploadProfilePhotoRequest(file))
        await _codutils.runcmd("rm -rf donottouch.jpg")
        await asyncio.sleep(Config.CHANGE_TIME)
...
codex.loop.create_task(autopfp_start())
codex.loop.create_task(autopicloop())
codex.loop.create_task(digitalpicloop())
codex.loop.create_task(bloom_pfploop())
codex.loop.create_task(autoname_loop())
codex.loop.create_task(autobio_loop())
codex.loop.create_task(custompfploop())
