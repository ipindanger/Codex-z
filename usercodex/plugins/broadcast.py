...
async def codbroadcast_add(event):
    ...
    codinput_str = event.pattern_match.group(1)
    if not codinput_str:
        return await edit_delete(
            event,
...
async def codbroadcast_list(event):
    ...
    await edit_or_reply(codevent, finaloutput)
...
async def codbroadcast_send(event):
...
async def codbroadcast_remove(event):
...
async def codbroadcast_delete(event):
    "To delete a category completely."
    codinput_str = event.pattern_match.group(1)
    check1 = sql.num_broadcastlist_chat(codinput_str)
    if check1 < 1:
        return await edit_delete(
            event,
            f"Are you sure that there is category {codinput_str}",
            parse_mode=_format.parse_pre,
        )
    try:
        sql.del_keyword_broadcastlist(codinput_str)
        await edit_or_reply(
            event,
            f"Successfully deleted the category {codinput_str}",
            parse_mode=_format.parse_pre,
        )
    except Exception as e:
        await edit_delete(
