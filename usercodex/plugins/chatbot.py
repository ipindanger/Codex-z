...
async def add_chatbot(event):
    ...
    codevent = await edit_or_reply(event, "`Adding ai to user...`")
    user, rank = await get_user_from_event(event, codevent, nogroup=True)
    if not user:
        return
    ...
    try:
        ...
        await edit_delete(codevent, f"**Error:**\n`{str(e)}`")
    else:
        await edit_or_reply(codevent, "Hi")
