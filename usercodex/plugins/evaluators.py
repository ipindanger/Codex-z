...
async def _(event):
    ...
    codevent = await edit_or_reply(event, "`Executing.....`")
    process = await asyncio.create_subprocess_shell(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    coduser = await event.client.get_me()
    curruser = coduser.username or "codexuserbot"
    uid = os.geteuid()
    if uid == 0:
...
@codex.cod_cmd(
    ...
    info={
        ...
        "examples": "{tr}eval print('codexuserbot')",
    },
)
async def _(event):
    ...
    codevent = await edit_or_reply(event, "`Running ...`")
