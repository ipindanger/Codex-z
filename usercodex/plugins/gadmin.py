...
async def codgban(event):  # sourcery no-metrics
    ...
    if reason:
        await code.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gbanned in {count} groups in {codtaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await code.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gbanned in {count} groups in {codtaken} seconds`!!"
...
async def codgban(event):
    ...
    if reason:
        await code.edit(
            f"[{user.first_name}](tg://user?id={user.id}`) was ungbanned in {count} groups in {codtaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await code.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was ungbanned in {count} groups in {codtaken} seconds`!!"
