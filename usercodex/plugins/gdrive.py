...
async def download(event, gdrive, service, uri=None):  # sourcery no-metrics
    ...
    if uri:
        ...
        if codtorrent:
            ...
            await edit_or_reply(
                gdrive,
                "`To use torrent files or download files from link install torrentutils from` @codplugins",
            )
...
async def g_download(event):
    ...
    file_name, codprocess = await gdrive_download(event, codevent, service, drive_link)
    if codprocess is not None:
        return await edit_delete(codevent, file_name)
    if os.path.exists(thumb_image_path):
        thumb = thumb_image_path
    if not cmd:
        await codevent.edit("**File Downloaded.\nLocation : **`" + str(file_name) + "`")
    else:
        c_time = time.time()
        await event.client.send_file(
            ...
            progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                progress(d, t, codevent, c_time, "Uploading...", file_name)
            ),
        )
        os.remove(file_name)
        await edit_delete(
...
async def gshare(event):
    ...
    codevent = await edit_or_reply(event, "`Creating sharable link...`")
    await asyncio.sleep(2)
    await share(service, codevent, input_str)
