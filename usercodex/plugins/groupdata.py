...
@codex.cod_cmd(
    ...
    info={
        ...
        "examples": "{tr}admins @CodexSupport",
    },
)
async def _(event):
...
async def get_users(show):
    ...
    codevent = await edit_or_reply(show, "`getting users list wait...`  ")
    try:
        ...
        mentions += " " + str(e) + "\n"
    await edit_or_reply(codevent, mentions)
...
@codex.cod_cmd(
    ...
    info={
        ...
        "examples": "{tr}chatinfo @CodexSupport",
    },
)
async def info(event):
    "To get group information"
    codevent = await edit_or_reply(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event, codevent)
    if chat is None:
        return
    caption = await fetch_info(chat, event)
    try:
        await codevent.edit(caption, parse_mode="html")
    except Exception as e:
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, f"**Error in chatinfo : **\n`{str(e)}`"
            )
        await codevent.edit("`An unexpected error has occurred.`")


async def get_chatinfo(event, codevent):
    ...
    try:
        ...
        try:
            ...
            await codevent.edit("`Invalid channel/group`")
            return None
            ...
            await codevent.edit("`Channel or supergroup doesn't exist`")
            return None
        except (TypeError, ValueError) as err:
            LOGS.info(err)
            await edit_delete(codevent, "**Error:**\n__Can't fetch the chat__")
            return None
    return chat_info
