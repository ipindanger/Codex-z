# Heroku manager for your CodexUserbot
...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "get":
        cod = await edit_or_reply(var, "`Getting information...`")
        ...
        cod = await edit_or_reply(var, "`Setting information...`")
        if not variable:
            return await cod.edit("`.set var <ConfigVars-name> <value>`")
        ...
        if variable in heroku_var:
            await cod.edit(f"`{variable}` **successfully changed to  ->  **`{value}`")
        else:
            await cod.edit(
        ...
        cod = await edit_or_reply(var, "`Getting information to deleting variable...`")
        try:
            variable = var.pattern_match.group(2).split()[0]
        except IndexError:
            return await cod.edit("`Please specify ConfigVars you want to delete`")
        await asyncio.sleep(1.5)
        if variable not in heroku_var:
            return await cod.edit(f"`{variable}`**  does not exist**")

        await cod.edit(f"`{variable}`  **successfully deleted**")
