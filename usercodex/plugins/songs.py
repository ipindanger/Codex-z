...
async def _(event):
    ...
    stderr = (await _codutils.runcmd(video_cmd))[1]
    if stderr:
        return await codevent.edit(f"**Error :** `{stderr}`")
    codname, stderr = (await _codutils.runcmd(name_cmd))[:2]
    if stderr:
    ...
    #    return await codevent.edit(f"**Error :** `{stderr}`")
    codname = os.path.splitext(codname)[0]
    vsong_file = Path(f"{codname}.mp4")
    if not os.path.exists(vsong_file):
        vsong_file = Path(f"{codname}.mkv")
    ...
    await codevent.delete()
    for files in (codthumb, vsong_file):
        if files and os.path.exists(files):
            os.remove(files)
...
async def shazamcmd(event):
    ...
    codevent = await edit_or_reply(event, "__Downloading the audio clip...__")
    try:
        ...
        LOGS.error(e)
        return await edit_delete(
            codevent, f"**Error while reverse searching song:**\n__{str(e)}__"
        )
...
async def _(event):
    ...
    codevent = await edit_or_reply(event, SONG_SEARCH_STRING, parse_mode="html")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if baka[0].message.startswith(
                ...
                return await edit_delete(
                    codevent, SONG_NOT_FOUND, parse_mode="html", time=5
                )
            await codevent.edit(SONG_SENDING_STRING, parse_mode="html")
            ...
            return await codevent.edit(SONGBOT_BLOCKED_STRING, parse_mode="html")
        await event.client.send_file(
            event.chat_id,
            music,
...
async def _(event):
    ...
    chat = "@auddbot"
    codevent = await edit_or_reply(event, "```Identifying the song```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await codevent.edit("Wait just a sec...")
            result = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
        except YouBlockedUserError:
            await codevent.edit("```Please unblock (@auddbot) and try again```")
            return
    namem = f"**Song Name : **`{result.text.splitlines()[0]}`\
        \n\n**Details : **__{result.text.splitlines()[2]}__"
    await codevent.edit(namem)
