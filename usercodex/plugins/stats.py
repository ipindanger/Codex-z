...
async def stats(event):  # sourcery no-metrics
    ...
    cod = await edit_or_reply(event, STAT_INDICATION)
...
async def stats(event):  # sourcery no-metrics
    codcmd = event.pattern_match.group(1)
    codevent = await edit_or_reply(event, STAT_INDICATION)
    start_time = time.time()
...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await edit_delete(codevent, "`unblock `@tgscanrobot` and then try again.`")
        response = await conv.get_response()
        await event.client.send_read_acknowledge(conv.chat_id)
        await codevent.edit(response.text)
