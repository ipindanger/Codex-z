...
class CodBroadcast(BASE):
    __tablename__ = "codbroadcast"
    ...
    def __repr__(self):
        return "<Cod Broadcast channels '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, CodBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


CodBroadcast.__table__.create(checkfirst=True)

CODBROADCAST_INSERTION_LOCK = threading.RLock()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(CodBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(CodBroadcast).all()
        for x in all_groups:
