...
class Cat_GlobalCollection(BASE):
    ...
    def __repr__(self):
        return "<Cod Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, Cod_GlobalCollection)
...
def add_to_collectionlist(keywoard, contents):
    with COD_GLOBALCOLLECTION:
        keyword_items = Cod_GlobalCollection(keywoard, tuple(contents))
...
def del_keyword_collectionlist(keywoard):
    with COD_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(Cod_GlobalCollection.keywoard)
            .filter(Cod_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(Cod_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(Cod_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(Cod_GlobalCollection).all()
        for x in all_groups:
