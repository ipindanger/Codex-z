...
class Cat_GlobalCollection_Json(BASE):
    __tablename__ = "cod_globalcollectionjson"
...
def get_collection(keywoard):
    try:
        return SESSION.query(Cod_GlobalCollection_Json).get(keywoard)
    finally:
        SESSION.close()
...
def add_collection(keywoard, json, njson):
    ...
    if not to_check:
        keyword_items = Cod_GlobalCollection_Json(keywoard, json, njson)
...
def del_collection(keywoard):
    ...
    if not to_check:
        return False
    keyword_items = SESSION.query(Cod_GlobalCollection_Json).get(keywoard)
...
def get_collections():
    try:
        return SESSION.query(Cod_GlobalCollection_Json).all()
    finally:
        SESSION.close()
