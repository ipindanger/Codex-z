...
class CatGloballist(BASE):
    ...
    def __repr__(self):
        return "<Cod global values '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, CodGloballist)
...
def add_to_list(keywoard, group_id):
    with CODGLOBALLIST_INSERTION_LOCK:
        broadcast_group = CodGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with CODGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(CodGloballist).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_list(keywoard):
    with CODGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(CodGloballist.keywoard)
            .filter(CodGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(CodGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(CodGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(CodGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(CodGloballist).all()
        for x in all_groups:
